...
testUpload()
  .then(result => {
    ...
    if (result.status === 401 && result.data.code === 401) {
      console.log('\n✅ 测试通过');
      console.log('✅ 后端正确返回401响应');
      console.log('✅ 响应格式: { code, message }');
      console.log('✅ 错误消息:', result.data.message);
      console.log('\n前端应该能够:');
      console.log('1. 正确接收401响应（不会出现ERR_CONNECTION_RESET）');
      console.log('2. 在文件列表中显示错误状态');
      console.log('3. 显示正确的错误消息');
      console.log('4. 统计时正确识别为失败（不会显示"全部上传完成"）');
    } else {
      console.log('\n⚠️ 响应异常');
    }
  })
