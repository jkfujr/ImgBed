...
class StorageMaintenanceScheduler {
        ...
        const scheduleNext = () => {
            this.compensationRetryTimer = setTimeout(async () => {
                try {
                    ...
                    this.log.error({ err }, 'scheduled compensation retry failed');
                    this.compensationBackoffMs = Math.min(
                        this.compensationBackoffMs * 2,
                        MAX_INTERVAL_MS
                    );
    ...
    startFullRebuildTimer() {
        ...
        this.fullRebuildTimer = setInterval(async () => {
            try {
                this.log.info('scheduled quota consistency check started');
                ...
                this.log.error({ err }, 'scheduled quota maintenance failed');
            }
        }, intervalMs);
