...
class StorageOperationRecovery {
    ...
    async recoverPendingOperations({ limit = 50 } = {}) {
        ...
        try {
            ...
            this.log.info({ count: operations.length }, 'recovery scan found stale storage operations');
    ...
    async executeRecovery(operation) {
        ...
        if (retryCount >= MAX_RETRIES) {
            markOperationFailed(db, operation.id, new Error(`retries exceeded max limit ${MAX_RETRIES}`));
            this.log.warn({ operationId: operation.id, retryCount }, 'recovery aborted after max retries');
            return;
        }
        ...
        try {
            ...
            this.log.error({ operationId: operation.id, retryCount: retryCount + 1, err }, 'recovery execution failed');
        }
    }
    ...
    async recoverRemoteDoneOperation(operation) {
        ...
        markOperationCompleted(db, operation.id);
        this.log.info({ operationId: operation.id }, 'recovered remote_done delete operation');
    }
    ...
    async recoverCommittedOperation(operation) {
        ...
        markOperationCompleted(db, operation.id);
        this.log.info({ operationId: operation.id }, 'recovered committed operation');
    }
    ...
    async executeCompensation(operation, { payloadField = 'compensation_payload' } = {}) {
        ...
        markOperationCompensated(db, operation.id, { compensationPayload: payload });
        this.log.info({ operationId: operation.id }, 'compensation completed');
    }
}
