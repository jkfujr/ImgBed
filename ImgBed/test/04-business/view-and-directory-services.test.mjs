...
test('parseRangeHeader 和 buildStreamHeaders 会生成当前访问链需要的部分内容头', () => {
  ...
  assert.equal(headers.get('content-disposition'), "inline; filename*=UTF-8''demo.png");
...
test('handleRegularStream 会优先使用上游图片 content-type', async () => {
  const res = new MockResponse();
  const storage = {
    async getStreamResponse() {
      return createStorageReadResult({
        stream: Readable.from([Buffer.from('image-body')]),
        contentLength: 10,
        totalSize: 10,
        statusCode: 200,
        acceptRanges: true,
  ...
  assert.equal(res.body, 'image-body');
});

test('handleRegularStream 在上游返回非图片 content-type 时回退到数据库图片类型', async () => {
  const res = new MockResponse();
  const storage = {
    async getStreamResponse() {
      return createStorageReadResult({
        stream: Readable.from([Buffer.from('webp-body')]),
        contentLength: 9,
        totalSize: 9,
        statusCode: 200,
        acceptRanges: true,
  ...
  assert.equal(res.statusCode, 200);
  assert.equal(res.getHeader('content-type'), 'image/webp');
  assert.equal(res.getHeader('content-disposition'), "inline; filename*=UTF-8''demo.webp");
  assert.equal(res.body, 'webp-body');
});
