...
test('认证、公开配置与访客上传边界会按当前路由装配返回结果', () => {
  const script = `
    ...
    try {
      ...
      const uploadWithoutTicket = await requestJson('/api/upload', {
        method: 'POST',
      });
      const legacyHeaderUpload = await requestJson('/api/upload', {
        method: 'POST',
        headers: {
          'X-Upload-Password': 'guest-secret',
        },
      });
      const wrongPassword = await requestJson('/api/public/guest-upload-ticket', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password: 'wrong-password' }),
      });
      const guestTicket = await requestJson('/api/public/guest-upload-ticket', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password: 'guest-secret' }),
      });
      const ticketCookie = guestTicket.headers.get('set-cookie') || '';
      const guestEnabledWithTicket = await requestJson('/api/public/guest-upload-config', {
        headers: {
          Cookie: ticketCookie,
        },
      });
      ...
      const updateGuestUploadPassword = await requestJson('/api/system/config', {
        method: 'PUT',
        headers: {
          Authorization: \`Bearer \${token}\`,
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          security: {
            uploadPassword: 'guest-secret-next',
          },
        }),
      });
      const uploadWithStaleTicket = await requestJson('/api/upload', {
        method: 'POST',
        headers: {
          Cookie: ticketCookie,
        },
      ...
      console.log('JSON_RESULT ' + JSON.stringify({
        ...
        guestTicket: {
          ...
          body: guestTicket.body,
          setCookie: ticketCookie,
        },
        guestEnabledWithTicket,
        noFile,
        updateGuestUploadPassword,
        uploadWithStaleTicket,
      }));
    } finally {
  ...
  assert.equal(payload.uploadWithoutTicket.status, 401);
  assert.equal(payload.uploadWithoutTicket.body.reason, 'AUTH_UPLOAD_PASSWORD_REQUIRED');
  assert.equal(payload.legacyHeaderUpload.status, 401);
  assert.equal(payload.legacyHeaderUpload.body.reason, 'AUTH_UPLOAD_PASSWORD_REQUIRED');
  assert.equal(payload.wrongPassword.status, 401);
  assert.equal(payload.wrongPassword.body.reason, 'AUTH_UPLOAD_PASSWORD_WRONG');
  assert.equal(payload.guestTicket.status, 200);
  assert.match(payload.guestTicket.setCookie, /imgbed_guest_upload_ticket=/);
  assert.match(payload.guestTicket.setCookie, /HttpOnly/);
  assert.match(payload.guestTicket.setCookie, /SameSite=Lax/);
  assert.deepEqual(payload.guestEnabledWithTicket.body.data, {
    guestUploadEnabled: true,
    requirePassword: true,
  ...
  assert.equal(payload.updateGuestUploadPassword.status, 200);
  assert.equal(payload.uploadWithStaleTicket.status, 401);
  assert.equal(payload.uploadWithStaleTicket.body.reason, 'AUTH_UPLOAD_PASSWORD_REQUIRED');
});
