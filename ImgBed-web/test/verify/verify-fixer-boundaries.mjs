...
const repoRoot = path.resolve(process.cwd(), 'ImgBed', 'ImgBed-web');
const testDir = path.join(repoRoot, 'test');
const fixerPath = path.join(testDir, 'lib', 'fixer.mjs');
const runPath = path.join(testDir, 'run.mjs');
...
function testRunUsesCurrentReporterAfterFix() {
  ...
  assert(content.includes('currentFiles = newFiles;'), 'run.mjs 未在修复后切换当前文件列表');
  assert(content.includes('currentReporter = newReporter;'), 'run.mjs 未在修复后切换当前报告器');
  assert(content.includes('const mdContent = currentReporter.toMarkdown(currentFiles.length, ruleList);'), 'run.mjs Markdown 报告未使用修复后结果');
  assert(content.includes('if (args.exitOnError && currentReporter.hasErrors()) {'), 'run.mjs exit code 判定未使用 currentReporter');
...
async function main() {
  const tests = [
    testRunUsesCurrentReporterAfterFix,
