...
function testExternalChecksModuleExists() {
  const externalChecks = read('test/lib/external-checks.mjs');
  ...
  assert(externalChecks.includes('npm run lint'), 'runESLint 应调用 npm run lint');
  assert(externalChecks.includes('npx tsc --noEmit'), 'runTypeScript 应调用 npx tsc --noEmit');
...
function testRunMjsIntegratesExternalChecks() {
  const runMjs = read('test/run.mjs');

  assert(runMjs.includes("from './lib/external-checks.mjs'"), 'run.mjs 应导入 external-checks.mjs');
  ...
  assert(runMjs.includes('!externalResult.allPassed'), 'run.mjs 应在 --exit-on-error 时考虑外部检查结果');

  return { name: 'run.mjs 已集成外部静态检查', ok: true };
}

function testReporterSupportsExternalChecks() {
  const reporter = read('test/lib/reporter.mjs');
